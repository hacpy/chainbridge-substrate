...
fn set_get_threshold() {
    new_test_ext().execute_with(|| {
        ...
        assert_events(vec![
            Event::bridge(Event::RelayerThresholdChanged(TEST_THRESHOLD)),
            Event::bridge(Event::RelayerThresholdChanged(5)),
        ]);
    })
}
...
fn add_remove_relayer() {
    new_test_ext().execute_with(|| {
        ...
        assert_events(vec![
            Event::bridge(Event::RelayerAdded(RELAYER_A)),
            Event::bridge(Event::RelayerAdded(RELAYER_B)),
            Event::bridge(Event::RelayerAdded(RELAYER_C)),
            Event::bridge(Event::RelayerRemoved(RELAYER_B)),
        ]);
    })
}
...
fn create_sucessful_proposal() {
    ...
    new_test_ext_initialized(src_id, r_id, b"System.remark".to_vec()).execute_with(|| {
        ...
        assert_events(vec![
            Event::bridge(Event::VoteFor(src_id, prop_id, RELAYER_A)),
            Event::bridge(Event::VoteAgainst(src_id, prop_id, RELAYER_B)),
            Event::bridge(Event::VoteFor(src_id, prop_id, RELAYER_C)),
            Event::bridge(Event::ProposalApproved(src_id, prop_id)),
            Event::bridge(Event::ProposalSucceeded(src_id, prop_id)),
        ]);
    })
}
...
fn create_unsucessful_proposal() {
    ...
    new_test_ext_initialized(src_id, r_id, b"System.remark".to_vec()).execute_with(|| {
        ...
        assert_events(vec![
            Event::bridge(Event::VoteFor(src_id, prop_id, RELAYER_A)),
            Event::bridge(Event::VoteAgainst(src_id, prop_id, RELAYER_B)),
            Event::bridge(Event::VoteAgainst(src_id, prop_id, RELAYER_C)),
            Event::bridge(Event::ProposalRejected(src_id, prop_id)),
        ]);
    })
}
...
fn execute_after_threshold_change() {
    ...
    new_test_ext_initialized(src_id, r_id, b"System.remark".to_vec()).execute_with(|| {
        ...
        assert_events(vec![
            Event::bridge(Event::VoteFor(src_id, prop_id, RELAYER_A)),
            Event::bridge(Event::RelayerThresholdChanged(1)),
            Event::bridge(Event::ProposalApproved(src_id, prop_id)),
            Event::bridge(Event::ProposalSucceeded(src_id, prop_id)),
        ]);
    })
...
fn proposal_expires() {
    ...
    new_test_ext_initialized(src_id, r_id, b"System.remark".to_vec()).execute_with(|| {
        ...
        assert_events(vec![Event::bridge(Event::VoteFor(
